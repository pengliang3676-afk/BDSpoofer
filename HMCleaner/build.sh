...
HELPER="$PKG/usr/local/bin/hmcleaner"
...
codesign --force --sign - --timestamp=none --entitlements HMCleaner/HMCleaner.entitlements "$HELPER"
...
xcrun lipo "$HELPER" -verify_arch arm64 arm64e
...
shasum -a 256 HMCleaner/HMCleaner.m HMCleaner/hmcleaner-standalone.m \
    "$APP/HMCleaner" "$HELPER" "$OUT/HMCleaner_${VERSION}_RootHide.deb" > "$OUT/SHA256SUMS.txt"
xcrun lipo -archs "$APP/HMCleaner" > "$OUT/app-archs.txt"
xcrun lipo -archs "$HELPER" > "$OUT/helper-archs.txt"
codesign -d --entitlements :- "$APP" > "$OUT/app-entitlements.txt" 2>&1
codesign -d --entitlements :- "$HELPER" > "$OUT/helper-entitlements.txt" 2>&1
