//
//  BDDiag10.m —— 百度极速版 6.96.0 登录 DI 3/17/27 定向放行验证版
//
//  只在 plainDeviceInfoWithInterface: 正在处理精确路径 /v3/login/api/auth 时，
//  将 3(deviceModel)、17(deviceType)、27(deviceName) 的最终排除结果改为 NO。
//  不修改 dt_params 字典、其他索引、其他接口或字段内容。
//
...
static _Atomic(unsigned) g_b9PrivacyLoggedGeneration = 0;
...
static _Thread_local int t_b10LoginDIAllowDepth = 0;
...
static const NSTimeInterval kB10CaptureSeconds = 600.0;
...
static BOOL b9_isObjectPointerEncoding(const char *encoding) {
    if (!encoding) return NO;
    while (*encoding && (isdigit((unsigned char)*encoding) || *encoding == 'r' || *encoding == 'n' ||
                         *encoding == 'N' || *encoding == 'o' || *encoding == 'O' ||
                         *encoding == 'R' || *encoding == 'V')) encoding++;
    if (*encoding != '^') return NO;
    encoding++;
    while (*encoding && (*encoding == 'r' || *encoding == 'n' || *encoding == 'N' ||
...
static NSString *b6_webProbeScript(void) {
    ...
    dispatch_once(&once, ^{
        ...
        @"(function(){try{"
         ...
         @"if(!window.__bd10){var st={events:[],seen:{},of:window.fetch,xo:null,xs:null};window.__bd10=st;"
         ...
         @"XMLHttpRequest.prototype.open=function(m,u){this.__bd10u=clean(u);this.__bd10m=String(m||'GET');return st.xo.apply(this,arguments)};"
         @"XMLHttpRequest.prototype.send=function(body){if(!this.__bd10a){this.__bd10a=1;this.__bd10q=keysOfBody(body);this.__bd10q.method=this.__bd10m||'GET';this.addEventListener('loadend',function(){"
         @"var t='',o=null,rt=String(this.responseType||'text');try{if(rt===''||rt==='text')t=String(this.responseText||'');else if(rt==='json'){o=this.response;t=JSON.stringify(o)||''}}catch(e){}st.add('xhr',this.__bd10u||this.responseURL,this.status,t,o,this.__bd10q,rt)})}"
         @"return st.xs.apply(this,arguments)}}"
         @"window.__bd10Restore=function(){try{if(st.of)window.fetch=st.of;if(st.xo)XMLHttpRequest.prototype.open=st.xo;"
         @"if(st.xs)XMLHttpRequest.prototype.send=st.xs;delete window.__bd10;delete window.__bd10Restore}catch(e){}}}"
         ...
         @"return JSON.stringify({href:clean(location.href),titleLen:String(document.title||'').length,markers:mk,userAgent:ua(),resources:rs,events:window.__bd10.events||[]})"
...
static void b6_removeWebInstrumentation(void) {
    NSString *cleanup = @"(function(){try{if(window.__bd10Restore)window.__bd10Restore()}catch(e){}})();";
...
static void b4_updateStatusAsync(void) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (b4_isCapturing())
            ...
            g_b4StatusLabel.text = [NSString stringWithFormat:@"BDDiag10 待采集 H%lu / E%u",
                                    (unsigned long)hooks, MIN(count, kB4MaxEvents)];
    });
}
...
static id b4_originalDeviceModel(id helperClass) {
    SEL alias = sel_registerName("bd10orig_deviceModel");
...
static id b6_originalDeviceName(id helperClass) {
    SEL alias = sel_registerName("bd10orig_deviceName");
    if (![helperClass respondsToSelector:alias]) return nil;
    return ((id (*)(id, SEL))objc_msgSend)(helperClass, alias);
}

static id b6_originalDeviceType(id helperClass) {
    SEL alias = sel_registerName("bd10orig_deviceType");
    if (![helperClass respondsToSelector:alias]) return nil;
    return ((id (*)(id, SEL))objc_msgSend)(helperClass, alias);
}
...
static BOOL b9_enablePrivacyMappingService(id self, SEL command, id __autoreleasing *outParams) {
    SEL alias = sel_registerName("bd10orig_enablePrivacyMappingService:");
    BOOL enabled = ((BOOL (*)(id, SEL, id __autoreleasing *))objc_msgSend)(self, alias, outParams);
    if (b4_isCapturing()) @try {
        id params = outParams ? *outParams : nil;
        id dtParams = [params isKindOfClass:NSDictionary.class]
            ? [(NSDictionary *)params objectForKey:@"dt_params"] : nil;
        NSArray *snapshot = b8_collectionSnapshot(dtParams);
        if (enabled && snapshot) {
            unsigned generation = atomic_load_explicit(&g_b4Generation, memory_order_acquire);
            unsigned seen = atomic_load_explicit(&g_b9PrivacyLoggedGeneration, memory_order_acquire);
            while (seen != generation) {
                if (atomic_compare_exchange_weak_explicit(&g_b9PrivacyLoggedGeneration, &seen, generation,
                                                          memory_order_acq_rel, memory_order_acquire)) {
                    b4_recordNoThrow(@"DI_PRIVACY_MAPPING_OUTPUT", b4_source(self, command),
                        [NSString stringWithFormat:@"enabled=YES; outPointer=%@; paramsShape=%@; feature=mapping; dt_params={%@}; contains3=%@; contains17=%@; contains27=%@; output=<numeric indexes only>",
                         outParams ? @"present" : @"NULL", b4_shape(params), b8_indexCollectionSummary(snapshot),
                         b8_collectionContainsIndex(snapshot, 3) ? @"YES" : @"NO",
                         b8_collectionContainsIndex(snapshot, 17) ? @"YES" : @"NO",
                         b8_collectionContainsIndex(snapshot, 27) ? @"YES" : @"NO"]);
                    break;
                }
            }
        }
...
static void b4_sendSms(id self, SEL command, id country, id phone, id captcha, id extra,
    ...
    @try {
        SEL alias = sel_registerName("bd10orig_sendSmsCodeWithCountryCode:phoneNumber:captcha:extraParams:success:failure:");
...
static void b4_smsLogin(id self, SEL command, id country, id phone, id smsCode, id encryptedId,
    ...
    @try {
        SEL alias = sel_registerName("bd10orig_smsLoginWithCountryCode:phoneNumber:smsCode:encryptedId:extraParams:success:verify:failure:");
...
static void b4_getDpass(id self, SEL command, id mobile, id captcha, id extra, id success, id failure) {
    ...
    @try {
        SEL alias = sel_registerName("bd10orig_getDpassWithMobile:captcha:extraParams:success:failure:");
...
static void b4_loginWithMobile(id self, SEL command, id mobile, id dpass, id extra, id success, id failure) {
    ...
    @try {
        SEL alias = sel_registerName("bd10orig_loginWithMobile:dpass:extraParams:success:failure:");
        ((void (*)(id, SEL, id, id, id, id, id))objc_msgSend)
...
static id b4_baseParams(id self, SEL command, id interfaceName) {
    SEL alias = sel_registerName("bd10orig_baseParamsForSMSLoginWithInterface:");
...
static BOOL b5_notAllowedGetDI(id self, SEL command, NSUInteger index) {
    SEL alias = sel_registerName("bd10orig_notAllowedGetDI:");
    BOOL originalResult = ((BOOL (*)(id, SEL, NSUInteger))objc_msgSend)(self, alias, index);
    BOOL override = t_b10LoginDIAllowDepth > 0 && (index == 3 || index == 17 || index == 27);
    BOOL effectiveResult = override ? NO : originalResult;
    if (b4_isCapturing() && (index == 3 || index == 17 || index == 27)) @try {
        ...
        NSString *sourceHint = !originalResult ? @"none" : (exceptContains ? @"deviceInfoExceptIndexArray" :
        ...
        b4_recordNoThrow(@"DI_DEVICE_FIELD_EXCLUSION", b4_source(self, command),
                         [NSString stringWithFormat:@"index=%lu(%@); originalExcluded=%@; loginOnlyOverride=%@; effectiveExcluded=%@; exceptArrayReadable=%@; exceptArrayContains=%@; exclusionSource=%@; DIdepth=%d; allowDepth=%d",
                          (unsigned long)index, field, originalResult ? @"YES" : @"NO",
                          override ? @"YES" : @"NO", effectiveResult ? @"YES" : @"NO",
                          exceptReadable ? @"YES" : @"NO", exceptContains ? @"YES" : @"NO",
                          sourceHint, t_b4DIDepth, t_b10LoginDIAllowDepth]);
        if (override)
            b4_recordNoThrow(@"DI_LOGIN_ALLOW_OVERRIDE", @"exact /v3/login/api/auth scope",
                [NSString stringWithFormat:@"index=%lu(%@); originalExcluded=%@; effectiveExcluded=NO; dt_params=<unchanged>",
                 (unsigned long)index, field, originalResult ? @"YES" : @"NO"]);
    } @catch (__unused NSException *e) { }
...
static id b8_valueForDI(id self, SEL command, NSUInteger index, id defaultValue) {
    SEL alias = sel_registerName("bd10orig_valueForDIWithIdx:def:");
...
static id b5_plainDeviceInfo(id self, SEL command, id interfaceName) {
    BOOL active = b4_isCapturing();
    BOOL loginAllowScope = [interfaceName isKindOfClass:NSString.class] &&
                           [(NSString *)interfaceName isEqualToString:@"/v3/login/api/auth"];
    if (active) t_b4DIDepth++;
    if (loginAllowScope) t_b10LoginDIAllowDepth++;
    id result = nil;
    @try {
        SEL alias = sel_registerName("bd10orig_plainDeviceInfoWithInterface:");
...
static id b5_generateDeviceInfo(id self, SEL command, id plainString) {
    SEL alias = sel_registerName("bd10orig_generateDeviceInfoWithPlainString:");
...
static id b5_deviceInfoString(id self, SEL command, id interfaceName) {
    ...
    id result = nil;
    @try {
        SEL alias = sel_registerName("bd10orig_deviceInfoStringWithInterface:");
        result = ((id (*)(id, SEL, id))objc_msgSend)(self, alias, interfaceName);
...
static id b5_interfaceForLogin(id self, SEL command) {
    SEL alias = sel_registerName("bd10orig_interfaceForLogin");
    id result = ((id (*)(id, SEL))objc_msgSend)(self, alias);
    if (b4_isCapturing()) @try {
...
static id b5_deviceInfoForLogin(id self, SEL command) {
    SEL alias = sel_registerName("bd10orig_deviceInfoForLogin");
    id result = ((id (*)(id, SEL))objc_msgSend)(self, alias);
    if (b4_isCapturing()) @try {
...
static id b4_request3(id self, SEL command, id method, id path, id parameters) {
    ...
    SEL alias = sel_registerName("bd10orig_requestWithMethod:path:parameters:");
...
static id b4_request4(id self, SEL command, id method, id path, double timeout, id parameters) {
    ...
    SEL alias = sel_registerName("bd10orig_requestWithMethod:path:timeout:parameters:");
...
static id b4_smsLoginURL(id self, SEL command) {
    SEL alias = sel_registerName("bd10orig_smsLoginURLString");
    id result = ((id (*)(id, SEL))objc_msgSend)(self, alias);
    if (b4_isCapturing()) @try {
...
static id b4_smsGetLoginURL(id self, SEL command) {
    SEL alias = sel_registerName("bd10orig_smsGetLoginURL");
    id result = ((id (*)(id, SEL))objc_msgSend)(self, alias);
    if (b4_isCapturing()) @try {
        b4_recordNoThrow(@"SMS_URL", b4_source(self, command), b4_urlSummary(result));
    } @catch (__unused NSException *e) { }
...
static id b6_webLoadRequest(id self, SEL command, id request) {
    ...
    SEL alias = sel_registerName("bd10orig_loadRequest:");
...
static id b6_webLoadHTML(id self, SEL command, id html, id baseURL) {
    ...
    SEL alias = sel_registerName("bd10orig_loadHTMLString:baseURL:");
...
static id b6_webReload(id self, SEL command) {
    ...
    SEL alias = sel_registerName("bd10orig_reload");
...
static id b6_webGoBack(id self, SEL command) {
    ...
    SEL alias = sel_registerName("bd10orig_goBack");
    return ((id (*)(id, SEL))objc_msgSend)(self, alias);
}
    ...
    @try {
        ...
        for (size_t i = 0; i < expectedCount; i++) {
            ...
            if (b4_typeKind(arg) != expectedArgs[i] ||
                (expectedArgs[i] == '^' && !b9_isObjectPointerEncoding(arg))) {
                b4_noteInstall([key stringByAppendingFormat:@" ABI rejected (%s)", types ?: "?"]);
                return NO;
            }
        }
...
static void b4_installAll(void) {
    b4_installExact("SAPILoginService", NO,
        ...
        "bd10orig_sendSmsCodeWithCountryCode:phoneNumber:captcha:extraParams:success:failure:",
        (IMP)b4_sendSms, 'v', "@@@@@@");
    b4_installExact("SAPILoginService", NO,
        "smsLoginWithCountryCode:phoneNumber:smsCode:encryptedId:extraParams:success:verify:failure:",
        "bd10orig_smsLoginWithCountryCode:phoneNumber:smsCode:encryptedId:extraParams:success:verify:failure:",
        (IMP)b4_smsLogin, 'v', "@@@@@@@@");
    b4_installExact("SAPILoginService", NO,
        "baseParamsForSMSLoginWithInterface:", "bd10orig_baseParamsForSMSLoginWithInterface:",
        (IMP)b4_baseParams, '@', "@");
    ...
    for (size_t i = 0; i < sizeof(loginClasses) / sizeof(loginClasses[0]); i++) {
        ...
        b4_installExact(name, NO, "getDpassWithMobile:captcha:extraParams:success:failure:",
            "bd10orig_getDpassWithMobile:captcha:extraParams:success:failure:",
            (IMP)b4_getDpass, 'v', "@@@@@");
        b4_installExact(name, NO, "loginWithMobile:dpass:extraParams:success:failure:",
            "bd10orig_loginWithMobile:dpass:extraParams:success:failure:",
            (IMP)b4_loginWithMobile, 'v', "@@@@@");
    }

    b4_installExact("SAPIHTTPRequest", YES, "requestWithMethod:path:parameters:",
        "bd10orig_requestWithMethod:path:parameters:", (IMP)b4_request3, '@', "@@@");
    b4_installExact("SAPIHTTPRequest", YES, "requestWithMethod:path:timeout:parameters:",
        "bd10orig_requestWithMethod:path:timeout:parameters:", (IMP)b4_request4, '@', "@@d@");
    b4_installExact("SAPIURLHelper", YES, "smsLoginURLString", "bd10orig_smsLoginURLString",
        (IMP)b4_smsLoginURL, '@', "");
    b4_installExact("SAPIURLHelper", YES, "smsGetLoginURL", "bd10orig_smsGetLoginURL",
        (IMP)b4_smsGetLoginURL, '@', "");

    b4_installExact("SAPIDeviceInfoHelper", YES, "deviceModel", "bd10orig_deviceModel",
        (IMP)b5_deviceModel, '@', "");
    b4_installExact("SAPIDeviceInfoHelper", YES, "deviceName", "bd10orig_deviceName",
        (IMP)b6_deviceName, '@', "");
    b4_installExact("SAPIDeviceInfoHelper", YES, "deviceType", "bd10orig_deviceType",
        (IMP)b6_deviceType, '@', "");
    b4_installExact("SAPIDeviceInfoHelper", YES, "enablePrivacyMappingService:",
        "bd10orig_enablePrivacyMappingService:", (IMP)b9_enablePrivacyMappingService, 'B', "^");
    b4_installExact("SAPIDeviceInfoHelper", YES, "notAllowedGetDI:", "bd10orig_notAllowedGetDI:",
        (IMP)b5_notAllowedGetDI, 'B', "Q");
    b4_installExact("SAPIDeviceInfoHelper", YES, "valueForDIWithIdx:def:",
        "bd10orig_valueForDIWithIdx:def:", (IMP)b8_valueForDI, '@', "Q@");
    b4_installExact("SAPIDeviceInfoHelper", YES, "plainDeviceInfoWithInterface:",
        "bd10orig_plainDeviceInfoWithInterface:", (IMP)b5_plainDeviceInfo, '@', "@");
    b4_installExact("SAPIDeviceInfoHelper", YES, "generateDeviceInfoWithPlainString:",
        "bd10orig_generateDeviceInfoWithPlainString:", (IMP)b5_generateDeviceInfo, '@', "@");
    b4_installExact("SAPIDeviceInfoHelper", YES, "deviceInfoStringWithInterface:",
        "bd10orig_deviceInfoStringWithInterface:", (IMP)b5_deviceInfoString, '@', "@");
    b4_installExact("SAPIDeviceInfoHelper", YES, "interfaceForLogin", "bd10orig_interfaceForLogin",
        (IMP)b5_interfaceForLogin, '@', "");
    b4_installExact("SAPIDeviceInfoHelper", YES, "deviceInfoForLogin", "bd10orig_deviceInfoForLogin",
        (IMP)b5_deviceInfoForLogin, '@', "");

    b4_installExact("WKWebView", NO, "loadRequest:", "bd10orig_loadRequest:",
        (IMP)b6_webLoadRequest, '@', "@");
    b4_installExact("WKWebView", NO, "loadHTMLString:baseURL:", "bd10orig_loadHTMLString:baseURL:",
        (IMP)b6_webLoadHTML, '@', "@@");
    b4_installExact("WKWebView", NO, "reload", "bd10orig_reload",
        (IMP)b6_webReload, '@', "");
    b4_installExact("WKWebView", NO, "goBack", "bd10orig_goBack",
        (IMP)b6_webGoBack, '@', "");
}
...
@interface BDDiag10Store : NSObject
+ (void)startCapture;
+ (void)stopCapture;
+ (void)exportReport;
...
@interface BDDiag10Window : UIWindow @end
@implementation BDDiag10Window
...
+ (void)startCapture {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(kB10CaptureSeconds * NSEC_PER_SEC)),
                       dispatch_get_main_queue(), ^{
            BOOL stopped = NO;
            os_unfair_lock_lock(&g_b4Lock);
            @try {
...
+ (NSString *)buildReport {
    ...
    [out appendString:@"BDDiag10 百度极速版 6.96.0 登录 DI 3/17/27 定向放行验证报告\n"];
    ...
    [out appendString:@"测试修改: 仅在精确接口 /v3/login/api/auth 的 DI 序列化动态范围内，将索引 3/17/27 的最终排除结果改为 NO；dt_params、其他索引和其他接口保持原样。\n\n"];
    ...
    if (events.count == 0)
        [out appendString:@"（采集窗口内未命中；请先点开始，再完成一次新登录，随后进入登录设备管理和设备详情。）\n"];
    return out;
}
...
+ (void)exportReport {
    ...
    @try {
        ...
        NSString *path = [docs stringByAppendingPathComponent:
            [NSString stringWithFormat:@"BDDiag10_log_%@.txt", [formatter stringFromDate:[NSDate date]]]];
        NSError *error = nil;
...
static BDDiag10Window *g_b4Window = nil;
...
static void b4_float(void) {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        t_b4Suppress++;
        @try {
            BDDiag10Window *window = [[BDDiag10Window alloc] initWithWindowScene:windowScene];
            ...
            status.text = [NSString stringWithFormat:@"BDDiag10 待采集 H%lu / E%u",
                           (unsigned long)b4_installedCount(),
                           MIN(atomic_load_explicit(&g_b4EventCount, memory_order_acquire), kB4MaxEvents)];
            ...
            [start addTarget:BDDiag10Store.class action:@selector(startCapture) forControlEvents:UIControlEventTouchUpInside];
            ...
            [export addTarget:BDDiag10Store.class action:@selector(exportReport) forControlEvents:UIControlEventTouchUpInside];
...
__attribute__((constructor)) static void bddiag10_entry(void) {
    @autoreleasepool {
        NSBundle *bundle = NSBundle.mainBundle;
        ...
        Dl_info info;
        memset(&info, 0, sizeof(info));
        if (dladdr((void *)&bddiag10_entry, &info) && info.dli_fbase) {
