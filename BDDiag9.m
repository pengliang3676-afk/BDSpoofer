//
//  BDDiag9.m —— 百度极速版 6.96.0 SAPI privacy mapping / dt_params 定点只读探针
//
//  在 BDDiag8 的 3/17/27 槽位证据上，直接捕获 enablePrivacyMappingService:
//  的对象输出参数及 dt_params 数字列表。探针不修改请求、响应或输出字典。
//
...
static NSString *b6_webProbeScript(void) {
    ...
    dispatch_once(&once, ^{
        ...
        @"(function(){try{"
         ...
         @"if(!window.__bd9){var st={events:[],seen:{},of:window.fetch,xo:null,xs:null};window.__bd9=st;"
         ...
         @"XMLHttpRequest.prototype.open=function(m,u){this.__bd9u=clean(u);this.__bd9m=String(m||'GET');return st.xo.apply(this,arguments)};"
         @"XMLHttpRequest.prototype.send=function(body){if(!this.__bd9a){this.__bd9a=1;this.__bd9q=keysOfBody(body);this.__bd9q.method=this.__bd9m||'GET';this.addEventListener('loadend',function(){"
         @"var t='',o=null,rt=String(this.responseType||'text');try{if(rt===''||rt==='text')t=String(this.responseText||'');else if(rt==='json'){o=this.response;t=JSON.stringify(o)||''}}catch(e){}st.add('xhr',this.__bd9u||this.responseURL,this.status,t,o,this.__bd9q,rt)})}"
         @"return st.xs.apply(this,arguments)}}"
         @"window.__bd9Restore=function(){try{if(st.of)window.fetch=st.of;if(st.xo)XMLHttpRequest.prototype.open=st.xo;"
         @"if(st.xs)XMLHttpRequest.prototype.send=st.xs;delete window.__bd9;delete window.__bd9Restore}catch(e){}}}"
         ...
         @"return JSON.stringify({href:clean(location.href),titleLen:String(document.title||'').length,markers:mk,userAgent:ua(),resources:rs,events:window.__bd9.events||[]})"
...
static void b6_removeWebInstrumentation(void) {
    NSString *cleanup = @"(function(){try{if(window.__bd9Restore)window.__bd9Restore()}catch(e){}})();";
...
static void b4_updateStatusAsync(void) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (b4_isCapturing())
            ...
            g_b4StatusLabel.text = [NSString stringWithFormat:@"BDDiag9 待采集 H%lu / E%u",
                                    (unsigned long)hooks, MIN(count, kB4MaxEvents)];
    });
}
...
static id b4_originalDeviceModel(id helperClass) {
    SEL alias = sel_registerName("bd9orig_deviceModel");
...
static id b6_originalDeviceName(id helperClass) {
    SEL alias = sel_registerName("bd9orig_deviceName");
    if (![helperClass respondsToSelector:alias]) return nil;
    return ((id (*)(id, SEL))objc_msgSend)(helperClass, alias);
}

static id b6_originalDeviceType(id helperClass) {
    SEL alias = sel_registerName("bd9orig_deviceType");
    if (![helperClass respondsToSelector:alias]) return nil;
    return ((id (*)(id, SEL))objc_msgSend)(helperClass, alias);
}
...
static BOOL b9_enablePrivacyMappingService(id self, SEL command, id __autoreleasing *outParams) {
    SEL alias = sel_registerName("bd9orig_enablePrivacyMappingService:");
...
static void b4_sendSms(id self, SEL command, id country, id phone, id captcha, id extra,
    ...
    @try {
        SEL alias = sel_registerName("bd9orig_sendSmsCodeWithCountryCode:phoneNumber:captcha:extraParams:success:failure:");
...
static void b4_smsLogin(id self, SEL command, id country, id phone, id smsCode, id encryptedId,
    ...
    @try {
        SEL alias = sel_registerName("bd9orig_smsLoginWithCountryCode:phoneNumber:smsCode:encryptedId:extraParams:success:verify:failure:");
...
static void b4_getDpass(id self, SEL command, id mobile, id captcha, id extra, id success, id failure) {
    ...
    @try {
        SEL alias = sel_registerName("bd9orig_getDpassWithMobile:captcha:extraParams:success:failure:");
...
static void b4_loginWithMobile(id self, SEL command, id mobile, id dpass, id extra, id success, id failure) {
    ...
    @try {
        SEL alias = sel_registerName("bd9orig_loginWithMobile:dpass:extraParams:success:failure:");
        ((void (*)(id, SEL, id, id, id, id, id))objc_msgSend)
...
static id b4_baseParams(id self, SEL command, id interfaceName) {
    SEL alias = sel_registerName("bd9orig_baseParamsForSMSLoginWithInterface:");
...
static BOOL b5_notAllowedGetDI(id self, SEL command, NSUInteger index) {
    SEL alias = sel_registerName("bd9orig_notAllowedGetDI:");
...
static id b8_valueForDI(id self, SEL command, NSUInteger index, id defaultValue) {
    SEL alias = sel_registerName("bd9orig_valueForDIWithIdx:def:");
...
static id b5_plainDeviceInfo(id self, SEL command, id interfaceName) {
    ...
    id result = nil;
    @try {
        SEL alias = sel_registerName("bd9orig_plainDeviceInfoWithInterface:");
...
static id b5_generateDeviceInfo(id self, SEL command, id plainString) {
    SEL alias = sel_registerName("bd9orig_generateDeviceInfoWithPlainString:");
...
static id b5_deviceInfoString(id self, SEL command, id interfaceName) {
    ...
    id result = nil;
    @try {
        SEL alias = sel_registerName("bd9orig_deviceInfoStringWithInterface:");
        result = ((id (*)(id, SEL, id))objc_msgSend)(self, alias, interfaceName);
    } @finally { if (active) t_b4DIDepth--; }
    if (active) @try {
...
static id b5_interfaceForLogin(id self, SEL command) {
    SEL alias = sel_registerName("bd9orig_interfaceForLogin");
    id result = ((id (*)(id, SEL))objc_msgSend)(self, alias);
    if (b4_isCapturing()) @try {
...
static id b5_deviceInfoForLogin(id self, SEL command) {
    SEL alias = sel_registerName("bd9orig_deviceInfoForLogin");
    id result = ((id (*)(id, SEL))objc_msgSend)(self, alias);
    if (b4_isCapturing()) @try {
...
static id b4_request3(id self, SEL command, id method, id path, id parameters) {
    ...
    SEL alias = sel_registerName("bd9orig_requestWithMethod:path:parameters:");
...
static id b4_request4(id self, SEL command, id method, id path, double timeout, id parameters) {
    ...
    SEL alias = sel_registerName("bd9orig_requestWithMethod:path:timeout:parameters:");
...
static id b4_smsLoginURL(id self, SEL command) {
    SEL alias = sel_registerName("bd9orig_smsLoginURLString");
    id result = ((id (*)(id, SEL))objc_msgSend)(self, alias);
    if (b4_isCapturing()) @try {
...
static id b4_smsGetLoginURL(id self, SEL command) {
    SEL alias = sel_registerName("bd9orig_smsGetLoginURL");
    id result = ((id (*)(id, SEL))objc_msgSend)(self, alias);
    if (b4_isCapturing()) @try {
        b4_recordNoThrow(@"SMS_URL", b4_source(self, command), b4_urlSummary(result));
    } @catch (__unused NSException *e) { }
...
static id b6_webLoadRequest(id self, SEL command, id request) {
    ...
    SEL alias = sel_registerName("bd9orig_loadRequest:");
...
static id b6_webLoadHTML(id self, SEL command, id html, id baseURL) {
    ...
    SEL alias = sel_registerName("bd9orig_loadHTMLString:baseURL:");
...
static id b6_webReload(id self, SEL command) {
    ...
    SEL alias = sel_registerName("bd9orig_reload");
...
static id b6_webGoBack(id self, SEL command) {
    ...
    SEL alias = sel_registerName("bd9orig_goBack");
    return ((id (*)(id, SEL))objc_msgSend)(self, alias);
}
...
static void b4_installAll(void) {
    b4_installExact("SAPILoginService", NO,
        ...
        "bd9orig_sendSmsCodeWithCountryCode:phoneNumber:captcha:extraParams:success:failure:",
        (IMP)b4_sendSms, 'v', "@@@@@@");
    b4_installExact("SAPILoginService", NO,
        "smsLoginWithCountryCode:phoneNumber:smsCode:encryptedId:extraParams:success:verify:failure:",
        "bd9orig_smsLoginWithCountryCode:phoneNumber:smsCode:encryptedId:extraParams:success:verify:failure:",
        (IMP)b4_smsLogin, 'v', "@@@@@@@@");
    b4_installExact("SAPILoginService", NO,
        "baseParamsForSMSLoginWithInterface:", "bd9orig_baseParamsForSMSLoginWithInterface:",
        (IMP)b4_baseParams, '@', "@");
    ...
    for (size_t i = 0; i < sizeof(loginClasses) / sizeof(loginClasses[0]); i++) {
        ...
        b4_installExact(name, NO, "getDpassWithMobile:captcha:extraParams:success:failure:",
            "bd9orig_getDpassWithMobile:captcha:extraParams:success:failure:",
            (IMP)b4_getDpass, 'v', "@@@@@");
        b4_installExact(name, NO, "loginWithMobile:dpass:extraParams:success:failure:",
            "bd9orig_loginWithMobile:dpass:extraParams:success:failure:",
            (IMP)b4_loginWithMobile, 'v', "@@@@@");
    }

    b4_installExact("SAPIHTTPRequest", YES, "requestWithMethod:path:parameters:",
        "bd9orig_requestWithMethod:path:parameters:", (IMP)b4_request3, '@', "@@@");
    b4_installExact("SAPIHTTPRequest", YES, "requestWithMethod:path:timeout:parameters:",
        "bd9orig_requestWithMethod:path:timeout:parameters:", (IMP)b4_request4, '@', "@@d@");
    b4_installExact("SAPIURLHelper", YES, "smsLoginURLString", "bd9orig_smsLoginURLString",
        (IMP)b4_smsLoginURL, '@', "");
    b4_installExact("SAPIURLHelper", YES, "smsGetLoginURL", "bd9orig_smsGetLoginURL",
        (IMP)b4_smsGetLoginURL, '@', "");

    b4_installExact("SAPIDeviceInfoHelper", YES, "deviceModel", "bd9orig_deviceModel",
        (IMP)b5_deviceModel, '@', "");
    b4_installExact("SAPIDeviceInfoHelper", YES, "deviceName", "bd9orig_deviceName",
        (IMP)b6_deviceName, '@', "");
    b4_installExact("SAPIDeviceInfoHelper", YES, "deviceType", "bd9orig_deviceType",
        (IMP)b6_deviceType, '@', "");
    b4_installExact("SAPIDeviceInfoHelper", YES, "enablePrivacyMappingService:",
        "bd9orig_enablePrivacyMappingService:", (IMP)b9_enablePrivacyMappingService, 'B', "^");
    b4_installExact("SAPIDeviceInfoHelper", YES, "notAllowedGetDI:", "bd9orig_notAllowedGetDI:",
        (IMP)b5_notAllowedGetDI, 'B', "Q");
    b4_installExact("SAPIDeviceInfoHelper", YES, "valueForDIWithIdx:def:",
        "bd9orig_valueForDIWithIdx:def:", (IMP)b8_valueForDI, '@', "Q@");
    b4_installExact("SAPIDeviceInfoHelper", YES, "plainDeviceInfoWithInterface:",
        "bd9orig_plainDeviceInfoWithInterface:", (IMP)b5_plainDeviceInfo, '@', "@");
    b4_installExact("SAPIDeviceInfoHelper", YES, "generateDeviceInfoWithPlainString:",
        "bd9orig_generateDeviceInfoWithPlainString:", (IMP)b5_generateDeviceInfo, '@', "@");
    b4_installExact("SAPIDeviceInfoHelper", YES, "deviceInfoStringWithInterface:",
        "bd9orig_deviceInfoStringWithInterface:", (IMP)b5_deviceInfoString, '@', "@");
    b4_installExact("SAPIDeviceInfoHelper", YES, "interfaceForLogin", "bd9orig_interfaceForLogin",
        (IMP)b5_interfaceForLogin, '@', "");
    b4_installExact("SAPIDeviceInfoHelper", YES, "deviceInfoForLogin", "bd9orig_deviceInfoForLogin",
        (IMP)b5_deviceInfoForLogin, '@', "");

    b4_installExact("WKWebView", NO, "loadRequest:", "bd9orig_loadRequest:",
        (IMP)b6_webLoadRequest, '@', "@");
    b4_installExact("WKWebView", NO, "loadHTMLString:baseURL:", "bd9orig_loadHTMLString:baseURL:",
        (IMP)b6_webLoadHTML, '@', "@@");
    b4_installExact("WKWebView", NO, "reload", "bd9orig_reload",
        (IMP)b6_webReload, '@', "");
    b4_installExact("WKWebView", NO, "goBack", "bd9orig_goBack",
        (IMP)b6_webGoBack, '@', "");
}
...
@interface BDDiag9Store : NSObject
+ (void)startCapture;
+ (void)stopCapture;
+ (void)exportReport;
...
@interface BDDiag9Window : UIWindow @end
@implementation BDDiag9Window
...
+ (NSString *)buildReport {
    ...
    [out appendString:@"BDDiag9 百度极速版 6.96.0 SAPI privacy mapping / dt_params 定点只读探针报告\n"];
...
+ (void)exportReport {
    ...
    @try {
        ...
        NSString *path = [docs stringByAppendingPathComponent:
            [NSString stringWithFormat:@"BDDiag9_log_%@.txt", [formatter stringFromDate:[NSDate date]]]];
        NSError *error = nil;
...
static BDDiag9Window *g_b4Window = nil;
...
static void b4_float(void) {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        t_b4Suppress++;
        @try {
            BDDiag9Window *window = [[BDDiag9Window alloc] initWithWindowScene:windowScene];
            ...
            status.text = [NSString stringWithFormat:@"BDDiag9 待采集 H%lu / E%u",
                           (unsigned long)b4_installedCount(),
                           MIN(atomic_load_explicit(&g_b4EventCount, memory_order_acquire), kB4MaxEvents)];
            ...
            [start addTarget:BDDiag9Store.class action:@selector(startCapture) forControlEvents:UIControlEventTouchUpInside];
            ...
            [export addTarget:BDDiag9Store.class action:@selector(exportReport) forControlEvents:UIControlEventTouchUpInside];
...
__attribute__((constructor)) static void bddiag9_entry(void) {
    @autoreleasepool {
        NSBundle *bundle = NSBundle.mainBundle;
        ...
        Dl_info info;
        memset(&info, 0, sizeof(info));
        if (dladdr((void *)&bddiag9_entry, &info) && info.dli_fbase) {
