...
for BDS_ARCH in arm64 arm64e; do
    xcrun --sdk iphoneos clang -arch "$BDS_ARCH" "${BDS_COMMON[@]}" "${BDS_FRAMEWORKS[@]}" -dynamiclib -install_name @rpath/BDSpoofer_1.8.1_UI1.1.dylib BDSpoofer.m -o "build-ui1/BDSpoofer_$BDS_ARCH.dylib"
...
lipo -create build-ui1/BDSpoofer_arm64.dylib build-ui1/BDSpoofer_arm64e.dylib -output dist-ui1/BDSpoofer_1.8.1_UI1.1.dylib
codesign --force --sign - --timestamp=none dist-ui1/BDSpoofer_1.8.1_UI1.1.dylib
...
(cd build-ui1/deb && ar -rc ../../dist-ui1/BDSpooferCraneManager_1.0.2-ui1_RootHide.deb debian-binary control.tar.gz data.tar.gz)
codesign --verify --strict dist-ui1/BDSpoofer_1.8.1_UI1.1.dylib
codesign --verify --strict "$BDS_APP"
lipo -info dist-ui1/BDSpoofer_1.8.1_UI1.1.dylib
lipo -info "$BDS_APP/BDSCraneManager"
otool -hv dist-ui1/BDSpoofer_1.8.1_UI1.1.dylib
otool -L dist-ui1/BDSpoofer_1.8.1_UI1.1.dylib
ar -t dist-ui1/BDSpooferCraneManager_1.0.2-ui1_RootHide.deb
tar -tzf build-ui1/deb/data.tar.gz
cp RELEASE_UI1.md bdspoofer_config.plist dist-ui1/
(cd dist-ui1 && shasum -a 256 BDSpoofer_1.8.1_UI1.1.dylib BDSpooferCraneManager_1.0.2-ui1_RootHide.deb > SHA256SUMS.txt)
