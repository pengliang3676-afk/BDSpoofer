...
for BDSD_ARCH in arm64 arm64e; do
    xcrun --sdk iphoneos clang -arch "$BDSD_ARCH" -isysroot "$BDSD_SDK" \
        ...
        -I "$BDSD_BUILD" -dynamiclib -install_name @rpath/BDSRewardDiagnostics_0.4.0.dylib \
...
lipo -create "$BDSD_BUILD/arm64.dylib" "$BDSD_BUILD/arm64e.dylib" \
    -output "$BDSD_DIST/BDSRewardDiagnostics_0.4.0.dylib"
codesign --force --sign - --timestamp=none "$BDSD_DIST/BDSRewardDiagnostics_0.4.0.dylib"
codesign --verify --strict "$BDSD_DIST/BDSRewardDiagnostics_0.4.0.dylib"
lipo "$BDSD_DIST/BDSRewardDiagnostics_0.4.0.dylib" -verify_arch arm64 arm64e
lipo -info "$BDSD_DIST/BDSRewardDiagnostics_0.4.0.dylib"
otool -L "$BDSD_DIST/BDSRewardDiagnostics_0.4.0.dylib"
...
(cd "$BDSD_DIST" && shasum -a 256 BDSRewardDiagnostics_0.4.0.dylib > SHA256SUMS.txt)
