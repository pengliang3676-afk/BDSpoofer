//
//  BDDiag7.m —— 百度极速版 7.14.0 登录设备写入链路定点只读探针
//
//  从退出前开始覆盖同账号重新登录，再观察 SAPI/DI、登录请求构造、WKWebView
//  fetch/XHR 请求形状与设备列表响应分类。探针不修改请求或响应。
//
...
static const NSTimeInterval kB7CaptureSeconds = 300.0;
...
static NSString *b6_webProbeScript(void) {
    ...
    dispatch_once(&once, ^{
        ...
        @"(function(){try{"
         ...
         @"if(!window.__bd7){var st={events:[],seen:{},of:window.fetch,xo:null,xs:null};window.__bd7=st;"
         ...
         @"XMLHttpRequest.prototype.open=function(m,u){this.__bd7u=clean(u);this.__bd7m=String(m||'GET');return st.xo.apply(this,arguments)};"
         @"XMLHttpRequest.prototype.send=function(body){if(!this.__bd7a){this.__bd7a=1;this.__bd7q=keysOfBody(body);this.__bd7q.method=this.__bd7m||'GET';this.addEventListener('loadend',function(){"
         @"var t='',o=null,rt=String(this.responseType||'text');try{if(rt===''||rt==='text')t=String(this.responseText||'');else if(rt==='json'){o=this.response;t=JSON.stringify(o)||''}}catch(e){}st.add('xhr',this.__bd7u||this.responseURL,this.status,t,o,this.__bd7q,rt)})}"
         @"return st.xs.apply(this,arguments)}}"
         @"window.__bd7Restore=function(){try{if(st.of)window.fetch=st.of;if(st.xo)XMLHttpRequest.prototype.open=st.xo;"
         @"if(st.xs)XMLHttpRequest.prototype.send=st.xs;delete window.__bd7;delete window.__bd7Restore}catch(e){}}}"
         ...
         @"return JSON.stringify({href:clean(location.href),titleLen:String(document.title||'').length,markers:mk,userAgent:ua(),resources:rs,events:window.__bd7.events||[]})"
...
static void b6_removeWebInstrumentation(void) {
    NSString *cleanup = @"(function(){try{if(window.__bd7Restore)window.__bd7Restore()}catch(e){}})();";
...
static void b4_updateStatusAsync(void) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (b4_isCapturing())
            ...
            g_b4StatusLabel.text = [NSString stringWithFormat:@"BDDiag7 待采集 H%lu / E%u",
                                    (unsigned long)hooks, MIN(count, kB4MaxEvents)];
    });
}
...
static id b4_originalDeviceModel(id helperClass) {
    SEL alias = sel_registerName("bd7orig_deviceModel");
...
static id b6_originalDeviceName(id helperClass) {
    SEL alias = sel_registerName("bd7orig_deviceName");
    if (![helperClass respondsToSelector:alias]) return nil;
    return ((id (*)(id, SEL))objc_msgSend)(helperClass, alias);
}

static id b6_originalDeviceType(id helperClass) {
    SEL alias = sel_registerName("bd7orig_deviceType");
    if (![helperClass respondsToSelector:alias]) return nil;
    return ((id (*)(id, SEL))objc_msgSend)(helperClass, alias);
}
...
static void b4_sendSms(id self, SEL command, id country, id phone, id captcha, id extra,
    ...
    @try {
        SEL alias = sel_registerName("bd7orig_sendSmsCodeWithCountryCode:phoneNumber:captcha:extraParams:success:failure:");
...
static void b4_smsLogin(id self, SEL command, id country, id phone, id smsCode, id encryptedId,
    ...
    @try {
        SEL alias = sel_registerName("bd7orig_smsLoginWithCountryCode:phoneNumber:smsCode:encryptedId:extraParams:success:verify:failure:");
...
static void b4_getDpass(id self, SEL command, id mobile, id captcha, id extra, id success, id failure) {
    ...
    @try {
        SEL alias = sel_registerName("bd7orig_getDpassWithMobile:captcha:extraParams:success:failure:");
...
static void b4_loginWithMobile(id self, SEL command, id mobile, id dpass, id extra, id success, id failure) {
    ...
    @try {
        SEL alias = sel_registerName("bd7orig_loginWithMobile:dpass:extraParams:success:failure:");
        ((void (*)(id, SEL, id, id, id, id, id))objc_msgSend)
...
static id b4_baseParams(id self, SEL command, id interfaceName) {
    SEL alias = sel_registerName("bd7orig_baseParamsForSMSLoginWithInterface:");
...
static id b6_deviceName(id self, SEL command) {
    ...
    if (b4_isCapturing() && t_b4DIDepth > 0) @try {
        b4_recordNoThrow(@"DI_DEVICE_NAME", b4_source(self, command),
            [NSString stringWithFormat:@"length=%lu; class=%@; value=<not logged>",
             (unsigned long)b4_textLength(result), b7_deviceStringClass(result)]);
    } @catch (__unused NSException *e) { }
    return result;
}
...
static BOOL b5_notAllowedGetDI(id self, SEL command, NSUInteger index) {
    SEL alias = sel_registerName("bd7orig_notAllowedGetDI:");
...
static id b5_plainDeviceInfo(id self, SEL command, id interfaceName) {
    ...
    id result = nil;
    @try {
        SEL alias = sel_registerName("bd7orig_plainDeviceInfoWithInterface:");
...
static id b5_generateDeviceInfo(id self, SEL command, id plainString) {
    SEL alias = sel_registerName("bd7orig_generateDeviceInfoWithPlainString:");
...
static id b5_deviceInfoString(id self, SEL command, id interfaceName) {
    ...
    id result = nil;
    @try {
        SEL alias = sel_registerName("bd7orig_deviceInfoStringWithInterface:");
        result = ((id (*)(id, SEL, id))objc_msgSend)(self, alias, interfaceName);
    } @finally { if (active) t_b4DIDepth--; }
    if (active) @try {
...
static id b5_interfaceForLogin(id self, SEL command) {
    SEL alias = sel_registerName("bd7orig_interfaceForLogin");
    id result = ((id (*)(id, SEL))objc_msgSend)(self, alias);
    if (b4_isCapturing()) @try {
...
static id b5_deviceInfoForLogin(id self, SEL command) {
    SEL alias = sel_registerName("bd7orig_deviceInfoForLogin");
    id result = ((id (*)(id, SEL))objc_msgSend)(self, alias);
    if (b4_isCapturing()) @try {
...
static id b4_request3(id self, SEL command, id method, id path, id parameters) {
    ...
    SEL alias = sel_registerName("bd7orig_requestWithMethod:path:parameters:");
...
static id b4_request4(id self, SEL command, id method, id path, double timeout, id parameters) {
    ...
    SEL alias = sel_registerName("bd7orig_requestWithMethod:path:timeout:parameters:");
...
static id b4_smsLoginURL(id self, SEL command) {
    SEL alias = sel_registerName("bd7orig_smsLoginURLString");
    id result = ((id (*)(id, SEL))objc_msgSend)(self, alias);
    if (b4_isCapturing()) @try {
...
static id b4_smsGetLoginURL(id self, SEL command) {
    SEL alias = sel_registerName("bd7orig_smsGetLoginURL");
    id result = ((id (*)(id, SEL))objc_msgSend)(self, alias);
    if (b4_isCapturing()) @try {
        b4_recordNoThrow(@"SMS_URL", b4_source(self, command), b4_urlSummary(result));
    } @catch (__unused NSException *e) { }
...
static id b6_webLoadRequest(id self, SEL command, id request) {
    ...
    SEL alias = sel_registerName("bd7orig_loadRequest:");
...
static id b6_webLoadHTML(id self, SEL command, id html, id baseURL) {
    ...
    SEL alias = sel_registerName("bd7orig_loadHTMLString:baseURL:");
...
static id b6_webReload(id self, SEL command) {
    ...
    SEL alias = sel_registerName("bd7orig_reload");
...
static id b6_webGoBack(id self, SEL command) {
    ...
    SEL alias = sel_registerName("bd7orig_goBack");
    return ((id (*)(id, SEL))objc_msgSend)(self, alias);
}
...
static void b4_installAll(void) {
    b4_installExact("SAPILoginService", NO,
        ...
        "bd7orig_sendSmsCodeWithCountryCode:phoneNumber:captcha:extraParams:success:failure:",
        (IMP)b4_sendSms, 'v', "@@@@@@");
    b4_installExact("SAPILoginService", NO,
        "smsLoginWithCountryCode:phoneNumber:smsCode:encryptedId:extraParams:success:verify:failure:",
        "bd7orig_smsLoginWithCountryCode:phoneNumber:smsCode:encryptedId:extraParams:success:verify:failure:",
        (IMP)b4_smsLogin, 'v', "@@@@@@@@");
    b4_installExact("SAPILoginService", NO,
        "baseParamsForSMSLoginWithInterface:", "bd7orig_baseParamsForSMSLoginWithInterface:",
        (IMP)b4_baseParams, '@', "@");
    ...
    for (size_t i = 0; i < sizeof(loginClasses) / sizeof(loginClasses[0]); i++) {
        ...
        b4_installExact(name, NO, "getDpassWithMobile:captcha:extraParams:success:failure:",
            "bd7orig_getDpassWithMobile:captcha:extraParams:success:failure:",
            (IMP)b4_getDpass, 'v', "@@@@@");
        b4_installExact(name, NO, "loginWithMobile:dpass:extraParams:success:failure:",
            "bd7orig_loginWithMobile:dpass:extraParams:success:failure:",
            (IMP)b4_loginWithMobile, 'v', "@@@@@");
    }

    b4_installExact("SAPIHTTPRequest", YES, "requestWithMethod:path:parameters:",
        "bd7orig_requestWithMethod:path:parameters:", (IMP)b4_request3, '@', "@@@");
    b4_installExact("SAPIHTTPRequest", YES, "requestWithMethod:path:timeout:parameters:",
        "bd7orig_requestWithMethod:path:timeout:parameters:", (IMP)b4_request4, '@', "@@d@");
    b4_installExact("SAPIURLHelper", YES, "smsLoginURLString", "bd7orig_smsLoginURLString",
        (IMP)b4_smsLoginURL, '@', "");
    b4_installExact("SAPIURLHelper", YES, "smsGetLoginURL", "bd7orig_smsGetLoginURL",
        (IMP)b4_smsGetLoginURL, '@', "");

    b4_installExact("SAPIDeviceInfoHelper", YES, "deviceModel", "bd7orig_deviceModel",
        (IMP)b5_deviceModel, '@', "");
    b4_installExact("SAPIDeviceInfoHelper", YES, "deviceName", "bd7orig_deviceName",
        (IMP)b6_deviceName, '@', "");
    b4_installExact("SAPIDeviceInfoHelper", YES, "deviceType", "bd7orig_deviceType",
        (IMP)b6_deviceType, '@', "");
    b4_installExact("SAPIDeviceInfoHelper", YES, "notAllowedGetDI:", "bd7orig_notAllowedGetDI:",
        (IMP)b5_notAllowedGetDI, 'B', "Q");
    b4_installExact("SAPIDeviceInfoHelper", YES, "plainDeviceInfoWithInterface:",
        "bd7orig_plainDeviceInfoWithInterface:", (IMP)b5_plainDeviceInfo, '@', "@");
    b4_installExact("SAPIDeviceInfoHelper", YES, "generateDeviceInfoWithPlainString:",
        "bd7orig_generateDeviceInfoWithPlainString:", (IMP)b5_generateDeviceInfo, '@', "@");
    b4_installExact("SAPIDeviceInfoHelper", YES, "deviceInfoStringWithInterface:",
        "bd7orig_deviceInfoStringWithInterface:", (IMP)b5_deviceInfoString, '@', "@");
    b4_installExact("SAPIDeviceInfoHelper", YES, "interfaceForLogin", "bd7orig_interfaceForLogin",
        (IMP)b5_interfaceForLogin, '@', "");
    b4_installExact("SAPIDeviceInfoHelper", YES, "deviceInfoForLogin", "bd7orig_deviceInfoForLogin",
        (IMP)b5_deviceInfoForLogin, '@', "");

    b4_installExact("WKWebView", NO, "loadRequest:", "bd7orig_loadRequest:",
        (IMP)b6_webLoadRequest, '@', "@");
    b4_installExact("WKWebView", NO, "loadHTMLString:baseURL:", "bd7orig_loadHTMLString:baseURL:",
        (IMP)b6_webLoadHTML, '@', "@@");
    b4_installExact("WKWebView", NO, "reload", "bd7orig_reload",
        (IMP)b6_webReload, '@', "");
    b4_installExact("WKWebView", NO, "goBack", "bd7orig_goBack",
        (IMP)b6_webGoBack, '@', "");
}
...
@interface BDDiag7Store : NSObject
+ (void)startCapture;
+ (void)stopCapture;
+ (void)exportReport;
...
@interface BDDiag7Window : UIWindow @end
@implementation BDDiag7Window
...
+ (void)startCapture {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(kB7CaptureSeconds * NSEC_PER_SEC)),
                       dispatch_get_main_queue(), ^{
            BOOL stopped = NO;
            os_unfair_lock_lock(&g_b4Lock);
            @try {
...
+ (NSString *)buildReport {
    ...
    [out appendString:@"BDDiag7 百度极速版登录设备写入链路定点只读探针报告\n"];
    ...
    [out appendString:@"隐私说明: 手机号/验证码/Cookie/Token/设备ID、DI 明文、UA 全文、HTML、请求与响应正文均不记录。\n\n"];
    ...
    if (events.count == 0)
        [out appendString:@"（采集窗口内未命中；请先点开始，再退出并重登同一账号，随后进入登录设备管理和未知设备详情。）\n"];
    return out;
}
...
+ (void)exportReport {
    ...
    @try {
        ...
        NSString *path = [docs stringByAppendingPathComponent:
            [NSString stringWithFormat:@"BDDiag7_log_%@.txt", [formatter stringFromDate:[NSDate date]]]];
        NSError *error = nil;
...
static BDDiag7Window *g_b4Window = nil;
...
static void b4_float(void) {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        t_b4Suppress++;
        @try {
            BDDiag7Window *window = [[BDDiag7Window alloc] initWithWindowScene:windowScene];
            ...
            status.text = [NSString stringWithFormat:@"BDDiag7 待采集 H%lu / E%u",
                           (unsigned long)b4_installedCount(),
                           MIN(atomic_load_explicit(&g_b4EventCount, memory_order_acquire), kB4MaxEvents)];
            ...
            [start addTarget:BDDiag7Store.class action:@selector(startCapture) forControlEvents:UIControlEventTouchUpInside];
            ...
            [export addTarget:BDDiag7Store.class action:@selector(exportReport) forControlEvents:UIControlEventTouchUpInside];
...
__attribute__((constructor)) static void bddiag7_entry(void) {
    @autoreleasepool {
        NSBundle *bundle = NSBundle.mainBundle;
        ...
        Dl_info info;
        memset(&info, 0, sizeof(info));
        if (dladdr((void *)&bddiag7_entry, &info) && info.dli_fbase) {
